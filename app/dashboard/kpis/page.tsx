...
function getColor(kpi: KPI) {
  ...
  const pct = kpi.esInverso ? (kpi.meta / Math.max(kpi.actual, 0.01)) * 100 : (kpi.actual / kpi.meta) * 100
...
function getPct(kpi: KPI) {
  ...
  return Math.min(100, Math.round(kpi.esInverso ? (kpi.meta / Math.max(kpi.actual, 0.01)) * 100 : (kpi.actual / kpi.meta) * 100))
}
...
export default function KPIsPage() {
  ...
  async function crearKPI() {
    ...
    const nuevo: KPI = {
      ...
      actual: 0, ratio: formRatio, descripcion: formDesc, estandar: false, esInverso: formEsInverso,
    }
    await supabase.from('kpis_empresa').insert({...nuevo, user_id: user.id})
    setKpis(prev => [...prev, nuevo])
    setFormNombre(''); setFormUnidad('porcentaje'); setFormMeta(''); setFormRatio(''); setFormDesc(''); setFormEsInverso(false)
    setShowCrear(false)
    setSaving(false)
  }
